...
class Payment_controller extends CI_Controller {
	...
	{
		parent::__construct();	
		$this->load->model('reports/payment_model','payment',TRUE);
    $this->load->helper(array('form', 'url','html'));
		$session_data = $this->session->userdata('logged_in');
		$this->data['user'] = $this->payment->get_profile();
		$this->data['restaurants'] = $this->payment->get_restaurant();  
    $this->load->library('picture');
		$this->load->library('currency');         
    @$this->data['reslogo'] = ($this->payment->get_rest_logo()=="")?base_url()."assets/images/logo3d.png":$this->payment->get_rest_logo();  
	...
	public function index()
	{
      ...
      $data['cur'] = $this->payment->get_currency($rest_id);  
			$data['payment'] = $this->payment->get_payment($rest_id,date('Y-m-d', strtotime($start_date)),date('Y-m-d', strtotime($end_date))); 
    ...
    $parsed = explode(",",$parsvars);  //var_dump($parsed);
		$data['restname'] = $this->payment->get_restaurant_name($parsed[2]); //(restid)
    @$data['reslogo'] = ($this->payment->get_restid_logo($parsed[2])=="")?base_url()."assets/images/logo3d.png":$this->payment->get_restid_logo($parsed[2]);  //(userid)  
  ...
  public function printing(){ 
	...
	{
		$data['profile'] = $this->payment->get_profile();
