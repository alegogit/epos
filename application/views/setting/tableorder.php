    ...
    <div class="row">
            ...
            <div class="table-responsive">
                ...
                <tr data-index="<?=$i?>" class="datarow" id="<?=$row->ID.'_'.$row->TABLE_NUMBER?>">
                  ...
                  <td style="">
                    <a id="TABLE_NUMBER-<?=$row->ID?>" class=""><?=$row->TABLE_NUMBER?></a>
                  </td>
                  <td style="">
                    <a id="POSITION-<?=$row->ID?>" class=""><?=$row->POSITION?></a>
                  </td>
    ...
    <div class="modal-content">
      ...
      ?>
        <div class="form-group" style="margin-bottom:10px"> 
          <div class="input-group">       
            <label for="inputCaption">Table Number</label>
            <input type="text" class="form-control" id="inputCaption" placeholder="please fill in a number format" name="tableorder_name" pattern="\d*" required>
          </div>
        </div><br />
        <div class="form-group" style="margin-bottom:10px"> 
          <div class="input-group">       
            <label for="inputCaption">Position</label>
            <input type="text" class="form-control" id="inputCaption" placeholder="please fill in a number format" name="tableorder_position" pattern="\d*" required>
          </div>
        </div><br />
        <div class="form-group" style="margin-bottom:10px"> 
          <div class="input-group">       
...
<?php  
  ...
  $edit_script .= "  $('#TABLE_NUMBER-".$row->ID."').editable({
                        url: '/process/tableorder?p=update',
                        pk: ".$row->ID.", 
                        validate: function(v) {
  	...
  	if(c) {
  	  ch.each(function(){
          ...
          var dataP = "idf="+idf;
  				$.ajax({
            type: "POST",
            url: "/process/tableorder?p=delete",
            data: dataP,
            cache: false,
            success: function(result){ 
              if(result.trim()!='OK'){    
